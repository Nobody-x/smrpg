...
int g_iAdrenalineTrailSprites[MAXPLAYERS+1] = {-1,...};

public Plugin myinfo = 
{
	name = "SM:RPG Upgrade > Adrenaline",
	author = "Jannik \"Peace-Maker\" Hartung",
	description = "Adrenaline upgrade for SM:RPG. Gain speed shortly when being shot.",
...
public void OnPluginStart()
{
	...
	LoadTranslations("smrpg_adrenaline_upgrades.phrases");
...
public void OnLibraryAdded(const char[] name)
{
	...
	{
		SMRPG_RegisterUpgradeType("Adrenaline", UPGRADE_SHORTNAME, "Gain speed for a short time when being shot.", 10, true, 5, 20, 20);
		...
		g_hCVDefaultSpeedIncrease = SMRPG_CreateUpgradeConVar(UPGRADE_SHORTNAME, "smrpg_adrenaline_speed_inc", "0.2", "Speed increase for each level when player is damaged.", 0, true, 0.1);
		g_hCVDefaultDuration = SMRPG_CreateUpgradeConVar(UPGRADE_SHORTNAME, "smrpg_adrenaline_duration", "0.8", "Duration of Adrenaline's effect in seconds.", 0, true, 0.1);
	}
}
...
public void OnMapStart()
{
	...
	if(!LoadWeaponConfig())
		LogError("Can't read config file in configs/smrpg/adrenaline_weapons.cfg!");
}
...
public void OnClientDisconnect(int client)
{
	...
	if(g_iAdrenalineTrailSprites[client] != -1 && IsValidEntity(g_iAdrenalineTrailSprites[client]))
		AcceptEntityInput(g_iAdrenalineTrailSprites[client], "Kill");
	g_iAdrenalineTrailSprites[client] = -1;
}
...
public void SMRPG_OnClientLaggedMovementReset(int client, LaggedMovementType type)
{
	if(type == LMT_Faster)
	{
		...
		{
			SetVariantString("");
			AcceptEntityInput(g_iAdrenalineTrailSprites[client], "SetParent"); //unset parent
		}
	}
}
...
public void Hook_OnTakeDamagePost(int victim, int attacker, int inflictor, float damage, int damagetype, int weapon, const float damageForce[3], const float damagePosition[3])
{
	...
	int iSprite = g_iAdrenalineTrailSprites[victim];
	if(iSprite == -1 || !IsValidEntity(iSprite))
	{
		...
		g_iAdrenalineTrailSprites[victim] = iSprite;
	}
...
bool LoadWeaponConfig()
{
	...
	BuildPath(Path_SM, sPath, sizeof(sPath), "configs/smrpg/adrenaline_weapons.cfg");
	
	if(!FileExists(sPath))
		return false;
	
	KeyValues hKV = new KeyValues("AdrenalineWeapons");
